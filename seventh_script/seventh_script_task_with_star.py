...
Метод переводит римскую запись числа в арабскую
        ...
        for i in range(len(self.roman_num)):
            if i == 0:
                result += roman_dict[self.roman_num[i]]
            else:
                if roman_dict[self.roman_num[i]] > roman_dict[self.roman_num[i - 1]]:
                    result += roman_dict[self.roman_num[i]] - 2 * roman_dict[self.roman_num[i - 1]]
                else:
                    result += roman_dict[self.roman_num[i]]
        return result
