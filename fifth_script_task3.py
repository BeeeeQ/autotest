...
def everything_for_your_cat(cats_data):
    """
Метод принимает список множеств и возвращает строку с сортировкой одинаковых значений в множествах
 по последним двум параметрам из множеств находя одинаковые
    :param cats_data: Списки множеств
    ...
    for data in cats_data:
        if not owners_dict.get(f'{data[-2]} {data[-1]}'):
            owners_dict[f'{data[-2]} {data[-1]}'] = f'{data[0]}, {data[1]}'
        else:
            owners_dict[f'{data[-2]} {data[-1]}'] = f'{owners_dict.get(f"{data[-2]} {data[-1]}")}; {data[0]}, {data[1]}'
    for owner, cat in owners_dict.items():
        our_str += f'{owner}: {cat}\n'
    return our_str
